<template>
  <div>
    <div class="row mt-5 d-flex justify-content-center">
      <div class="col-md-10 d-flex justify-content-center colorfondo">
        <h1 class="mt-5">
          !Trabajemos juntos¡
          <p class="mt-4">
            Si deseas consultar mayor información sobre nuestros servicios o
            tienes alguna duda al respecto, no dudes en solicitar una
            consultoría gratuita y uno de nuestros asesores te contactará a la
            brevedad.
          </p>
        </h1>
      </div>
      <div class="col-md-5 p-5 colorfondo">
        <div class="row colorfondo1">
          <div class="col-md-9">
            <div class="col-md-12">
              <h5>
                <strong>email: </strong
                ><a href="rrhh@360solucionesperu.com"
                  >rrhh@360solucionesperu.com</a
                >
              </h5>
              <br >
              <h5>
                <strong>Teléfono: </strong
                ><a href="tel:955462955">955 462 955</a>
              </h5>
              <h5>
                <strong>Dirección: </strong
                ><span style="color: black"
                  >C. Los Pinos 490, San Isidro 15073</span
                >
              </h5>
            </div>
          </div>
        </div>
        <div class="row d-flex justify-content-center pt-5">
          <div class="col-md-12 d-flex justify-content-center">
            <iframe
              src="https://www.google.com/maps/embed?pb=!1m18!1m12!1m3!1d7804.490251183233!2d-76.99601894609374!3d-12.026638299999997!2m3!1f0!2f0!3f0!3m2!1i1024!2i768!4f13.1!3m3!1m2!1s0x48bb46d92d8fc073%3A0xaf2bd3fe09ef417d!2s360%20Soluciones%20-%20Servicios%20M%C3%BAltiples!5e0!3m2!1ses-419!2spe!4v1656290982285!5m2!1ses-419!2spe"
              width="420"
              height="280"
              style="border: 0"
              allowfullscreen=""
              loading="lazy"
              referrerpolicy="no-referrer-when-downgrade"
            ></iframe>
          </div>
        </div>
      </div>
      <div class="col-md-5 d-flex p-5 justify-content-end colorfondo ">
        <div class="row ">
          <div class="col-md-10 bordertext">
            <h5>DEJANOS TUS DATOS:</h5>
          </div>
          <div class="col-md-12 d-flex justify-content-center colorfondo2 mt-4">
            <div class="row d-flex justify-content-center">
              <form action="" class="my-4" >
                <label for="disabledTextInput" class="text-center"></label>
                <input
                  type="text"
                  class="input"
                  placeholder="Nombre"
                  name="nom"
                >
                <label for="disabledTextInput"></label>
                <input
                  type="text"
                  class=""
                  placeholder="Apellidos"
                  name="ap"
                ><br ><br >
                <label for="disabledTextInput"></label>
                <input type="email" class="" placeholder="email" >
                <label for="disabledTextInput"></label>
                <input
                  type="number"
                  class=""
                  placeholder="Telefono"
                  max="9"
                  min="0"
                >

                <label for="exampleFormControlTextarea1"></label><br ><br >
                <textarea
                  class="form-control"
                  rows="3"
                  placeholder="Cuéntanos en que podemos ayudarte"
                ></textarea
                ><br >
                <div class="d-flex justify-content-center">
                  <button type="submit" class="btn btn-danger ps-3 px-3">
                    Enviar
                  </button>
                </div>
              </form>
            </div>
          </div>
          <div class="col-md-12 pt-4">
            <div class="row d-flex justify-content-end mt-3">
              <div class="col-md-9 bordertext">
                <h5>CHATEA POR WHATSAPP CON NUESTROS EJECUTIVOS</h5>
              </div>
              <div class="col-md-2">
                <h1>
                  <a href="https://wa.me/51934256086" target="_blank"
                    ><svg
                      style="width: 50px; height: 50px"
                      viewBox="0 0 24 24"
                      color="#00bb2d"
                    >
                      <path
                        fill="currentColor"
                        d="M12.04 2C6.58 2 2.13 6.45 2.13 11.91C2.13 13.66 2.59 15.36 3.45 16.86L2.05 22L7.3 20.62C8.75 21.41 10.38 21.83 12.04 21.83C17.5 21.83 21.95 17.38 21.95 11.92C21.95 9.27 20.92 6.78 19.05 4.91C17.18 3.03 14.69 2 12.04 2M12.05 3.67C14.25 3.67 16.31 4.53 17.87 6.09C19.42 7.65 20.28 9.72 20.28 11.92C20.28 16.46 16.58 20.15 12.04 20.15C10.56 20.15 9.11 19.76 7.85 19L7.55 18.83L4.43 19.65L5.26 16.61L5.06 16.29C4.24 15 3.8 13.47 3.8 11.91C3.81 7.37 7.5 3.67 12.05 3.67M8.53 7.33C8.37 7.33 8.1 7.39 7.87 7.64C7.65 7.89 7 8.5 7 9.71C7 10.93 7.89 12.1 8 12.27C8.14 12.44 9.76 14.94 12.25 16C12.84 16.27 13.3 16.42 13.66 16.53C14.25 16.72 14.79 16.69 15.22 16.63C15.7 16.56 16.68 16.03 16.89 15.45C17.1 14.87 17.1 14.38 17.04 14.27C16.97 14.17 16.81 14.11 16.56 14C16.31 13.86 15.09 13.26 14.87 13.18C14.64 13.1 14.5 13.06 14.31 13.3C14.15 13.55 13.67 14.11 13.53 14.27C13.38 14.44 13.24 14.46 13 14.34C12.74 14.21 11.94 13.95 11 13.11C10.26 12.45 9.77 11.64 9.62 11.39C9.5 11.15 9.61 11 9.73 10.89C9.84 10.78 10 10.6 10.1 10.45C10.23 10.31 10.27 10.2 10.35 10.04C10.43 9.87 10.39 9.73 10.33 9.61C10.27 9.5 9.77 8.26 9.56 7.77C9.36 7.29 9.16 7.35 9 7.34C8.86 7.34 8.7 7.33 8.53 7.33Z"
                      ></path>
                    </svg>
                  </a>
                </h1>
              </div>
            </div>
          </div>
        </div>
      </div>
    </div>
    <div class="row d-flex justify-content-center"></div>
  </div>
</template>
<style scoped>
.imgfondo {
  background-image: url("/imagenes/contact-fond.jpg");
  background-size: cover;
}
textarea {
  width: 100%;
}
input {
  width: 200px;
  padding: 4px;
  border-radius: 6px;
  border: 2px solid rgb(219, 132, 132);
}
textarea {
  border: 2px solid rgb(219, 132, 132);
}
p {
  font-size: 16px;
}
a {
  text-decoration: none;
  color: black;
}
h5 {
  color: rgb(34, 39, 105);
}
.colorfondo {
  background-color: #eee;
}
.colorfondo1 {
  background-color: rgb(252, 178, 178);
}
.colorfondo2 {
  background-color: rgb(211, 211, 211);
}
.bordertext{
  border: 2px solid rgb(173, 171, 171);
  background-color: rgb(209, 209, 209);
}
</style>